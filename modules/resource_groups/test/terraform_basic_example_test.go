...
func TestTerraformBasicExample(t *testing.T) {
	...
	expectedTags := map[string]string{
		"production": "awooga",
		"deleted":    "oh yeah",
		"important":  "your joking",
	}
	expectedLocation := "uksouth"
	expectedRGNames := []string{
		"wibble",
		"bibble",
	}
	...
	terraformOptions := terraform.WithDefaultRetryableErrors(t, &terraform.Options{
		...
		Vars: map[string]interface{}{
			...
			"resource_group_names": expectedRGNames,
			"location":             expectedLocation,
			"general_tags":         expectedTags,
		},
	...
	rgLocations := terraform.OutputList(t, terraformOptions, "resource_group_locations")
	assert.Equal(t, rgLocations[0], expectedLocation)
	rgNames := terraform.OutputList(t, terraformOptions, "resource_group_names")
	assert.Equal(t, expectedRGNames, rgNames)
	myTags := terraform.OutputListOfObjects(t, terraformOptions, "resource_group_tags")
	fmt.Printf(">>DEBUG\n%#v\n", myTags[0]["deleted"])
	assert.Equal(t, "oh yeah", myTags[0]["deleted"])
